...
7c. Continuous driver location streaming to the rider (depends on 7) [implemented]
   ...
   - Known limitation, explicitly accepted rather than solved: the TTL is the *only* stop-forwarding mechanism today, since no trip-start/trip-completion/cancellation event exists yet to clear the mapping early. `presence.Store.ClearActiveTrip` is written but intentionally unused — it's the hook a future trip-lifecycle phase should call. A location ping processed before its trip's `ride.assigned.v1` has been handled (no ordering guarantee across the two Kafka topics) is silently dropped and self-heals on the next ping — accepted, not engineered around.
...
- Not yet done: end-to-end manual verification of the continuous-streaming path against live Postgres/Kafka/Redis; the `ClearActiveTrip` hook has no caller yet (waiting on a future trip-start/trip-completion phase); committing/pushing these changes.
