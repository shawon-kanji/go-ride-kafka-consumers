...
7d. Driver "start trip" API (depends on 7c) [implemented]
   ...
   - Not built: trip completion/drop-off (separate future phase); any change to `trip_requests` (`assigned` already covers this — the further `in_progress` state lives entirely on `ongoing_trips`, per the Phase 1 frozen state machine).
