...
// Deliverer handles every trip-ended broadcast received from Redis on this
...
func (d *Deliverer) HandleBroadcast(ctx context.Context, payload []byte) {
	var broadcast TripEndedBroadcast
	if err := json.Unmarshal(payload, &broadcast); err != nil {
		log.Printf("websocket_gateway: discard unparseable trip ended broadcast: %v", err)
		return
	}

	riderID, err := uuid.Parse(broadcast.RiderID)
	if err != nil {
		log.Printf("websocket_gateway: discard trip ended broadcast with invalid rider_id=%q request_id=%s: %v", broadcast.RiderID, broadcast.RequestID, err)
		return
	}
	...
	message := ws.NewTripEndedMessage(ws.TripEndedParams{
	...
	if err != nil {
		log.Printf("websocket_gateway: marshal trip ended message failed request_id=%s: %v", broadcast.RequestID, err)
		return
	}
