...
// Deliverer handles every trip-started broadcast received from Redis on
...
func (d *Deliverer) HandleBroadcast(ctx context.Context, payload []byte) {
	var broadcast TripStartedBroadcast
	if err := json.Unmarshal(payload, &broadcast); err != nil {
		log.Printf("websocket_gateway: discard unparseable trip started broadcast: %v", err)
		return
	}

	riderID, err := uuid.Parse(broadcast.RiderID)
	if err != nil {
		log.Printf("websocket_gateway: discard trip started broadcast with invalid rider_id=%q request_id=%s: %v", broadcast.RiderID, broadcast.RequestID, err)
		return
	}
	...
	message := ws.NewTripStartedMessage(ws.TripStartedParams{
	...
	if err != nil {
		log.Printf("websocket_gateway: marshal trip started message failed request_id=%s: %v", broadcast.RequestID, err)
		return
	}
